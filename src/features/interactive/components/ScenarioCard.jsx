...
export default function ScenarioCard({ scenario }) {
  ...
  return (
    <article
      ...
      className="card-modern relative flex h-full flex-col justify-between rounded-lg border border-slate-200 bg-white p-6 shadow-sm transition"
      style={{
        borderLeft: '4px solid transparent'
      }}
    >
      {scenario.status === "locked" ? (
        <div className="absolute right-6 top-6 rounded-lg border border-slate-200 bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-500">
          Bloqueado
        </div>
      ) : null}
      ...
      <div className="mt-5 space-y-3">
        <button
          ...
          className={`nav-cta inline-flex w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition ${status.button.variant}`}
        >
          {status.button.label}
